...
class cnn_model(object):
    def __init__(self, epochs):
        self.layer_type = [0, 0, 1, 0, 0, 1, 2]   # 0 for conv layer, 1 for max pooling, 2 for fully connected
        self.epochs = epochs
        # filter size for different conv layers
        self.filter_size = [(10, 10), (8, 1), (0, 0), (4, 1), (4, 1), (0, 0), (0, 28)]
        self.strides = 1
        self.pooling_size = 2
        self.conv_depth = [1, 1, 1, 1, 1, 1, 1]   # depth of layers - always 1 for FC
        self.weights = {}
        self.bias = {}
        self.layer_inputs = {}
        self.layer_outputs = {}
    ...
    def max_pooling(self, layer_num, x_temp, conv_depth):
        ...
        for depth in range(conv_depth):
            ...
            final_output.append(np.asarray(temp_list))
            final_output_indices.append(np.asarray(temp_list_2).T)
            # final_output_indices.append(np.asarray(temp_list_2))
    ...
    def fc_feed_forward(self, layer_num, x_temp, weights):
        final_output = np.zeros((x_temp[0].shape[0], weights.shape[1]))
        for i in range(len(x_temp)):
            # Adding multiple depth output
            final_output = np.add(final_output, np.dot(x_temp[i], weights[:, :, i]))
    ...
    def backpropagate_FC(self, layer_num, x_temp, loss):
        deltaEOut = np.zeros((x_temp[0].shape[0], self.weights[layer_num][:, :, 0].shape[1]))
        deltaEOut.fill(loss)
